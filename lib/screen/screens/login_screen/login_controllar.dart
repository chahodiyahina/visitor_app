    ...
    try {
      ...
      if (response['error_description'] == null) {
        ...
        await LocalStorageServices.setDataToLocalStorage(
            ...
            stringData: loginModel.accessToken);
        await LocalStorageServices.setDataToLocalStorage(
        ...
        await LocalStorageServices.setDataToLocalStorage(
            ...
            integerData: loginModel.user?.id);
        await LocalStorageServices.setDataToLocalStorage(
            dataType: LocalStorageKey.stringType,
            prefKey: LocalStorageKey.email,
            stringData: loginModel.user?.email);
        await LocalStorageServices.setDataToLocalStorage(
            dataType: LocalStorageKey.stringType,
            prefKey: LocalStorageKey.name,
            stringData: loginModel.user?.name);
        await LocalStorageServices.setDataToLocalStorage(
            dataType: LocalStorageKey.stringType,
            prefKey: LocalStorageKey.userType,
            stringData: loginModel.user?.userType ?? "");
        await LocalStorageServices.setDataToLocalStorage(
            dataType: LocalStorageKey.stringType,
            prefKey: LocalStorageKey.gateNo,
            stringData: loginModel.user?.siteGateNumber ?? "");
        await LocalStorageServices.setDataToLocalStorage(
            dataType: LocalStorageKey.stringType,
            prefKey: LocalStorageKey.department,
            stringData: loginModel.user?.department ?? "");
        await LocalStorageServices.setDataToLocalStorage(
            dataType: LocalStorageKey.stringType,
            prefKey: LocalStorageKey.site,
            stringData: loginModel.user?.userSite ?? "");
        Get.offAllNamed(Routes.dashboardView);
        CustomToast.successToast(
            message: loginModel.message ?? "", context: context);
      } else {
        final decodedBody = jsonDecode(response['body']);
        customHideLoadingDialog();
        CustomToast.errorToast(
            message: decodedBody['message'], context: context);
